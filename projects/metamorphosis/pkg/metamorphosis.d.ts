 ...
 * counted.
 */
export function corner_counts(engine: WasmEngine): Uint32Array;
 ...
 */
export function create_lattice(count: number, mesh_id: number, cube_scale: number, stride: number): WasmEngine;
 ...
 */
export function live_archetypes(engine: WasmEngine): number;
 ...
 */
export function set_churn(engine: WasmEngine, stride: number): void;

/**
 * Migrations issued since the last call, and resets the counter.
 */
export function take_migrations(engine: WasmEngine): number;
...
export interface InitOutput {
    readonly corner_counts: (a: number) => [number, number];
    readonly create_lattice: (a: number, b: number, c: number, d: number) => number;
    readonly live_archetypes: (a: number) => number;
    readonly set_churn: (a: number, b: number) => void;
    readonly take_migrations: (a: number) => number;
