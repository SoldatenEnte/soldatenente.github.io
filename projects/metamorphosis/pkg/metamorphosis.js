/* @ts-self-types="./metamorphosis.d.ts" */
...
/**
 * Rows per signature, read straight off the archetype tables — index is the
 * same `Ember | Frost<<1 | Spark<<2` bitfield the corners use. This is not a
 * tally the demo maintains on the side; it is the engine's own storage,
 * counted.
 * @param {WasmEngine} engine
 * @returns {Uint32Array}
 */
export function corner_counts(engine) {
    _assertClass(engine, WasmEngine);
    const ret = wasm.corner_counts(engine.__wbg_ptr);
    var v1 = getArrayU32FromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 4, 4);
    return v1;
}
...
/**
 * Builds the lattice: `count` entities spread evenly over all eight
 * signatures.
 *
 * Each corner is spawned with its markers already in the bundle rather than
 * added afterwards. Both routes end in the same eight archetypes, but adding
 * components post-spawn would migrate every entity up to three times before
 * the first frame — paying, at load, precisely the cost this demo is built to
 * let you turn on and off yourself.
 * @param {number} count
 * @param {number} mesh_id
 * @param {number} cube_scale
 * @param {number} stride
 * @returns {WasmEngine}
 */
export function create_lattice(count, mesh_id, cube_scale, stride) {
    const ret = wasm.create_lattice(count, mesh_id, cube_scale, stride);
...
/**
 * Archetypes currently holding at least one row — including the camera and
 * light tables, which is why this reads 10 and not 8.
 * @param {WasmEngine} engine
 * @returns {number}
 */
export function live_archetypes(engine) {
    _assertClass(engine, WasmEngine);
    const ret = wasm.live_archetypes(engine.__wbg_ptr);
    return ret >>> 0;
}
...
/**
 * Live migration load. 0 freezes the lattice; smaller is heavier.
 * @param {WasmEngine} engine
 * @param {number} stride
 */
export function set_churn(engine, stride) {
    _assertClass(engine, WasmEngine);
    wasm.set_churn(engine.__wbg_ptr, stride);
}

/**
 * Migrations issued since the last call, and resets the counter.
 * @param {WasmEngine} engine
 * @returns {number}
 */
export function take_migrations(engine) {
    _assertClass(engine, WasmEngine);
    const ret = wasm.take_migrations(engine.__wbg_ptr);
    return ret >>> 0;
}
...
async function __wbg_init(module_or_path, memory) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('metamorphosis_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports(memory);
