...
import { WORLD_BOUNDARY } from "../config/constants.ts";
...
export const PlayerShip = forwardRef<THREE.Group, PlayerShipProps>(
    ({ waterUniforms, waterLevel, shipScale, shipDimensions, updateSailingSound, isPhysicsPaused }, ref) => {
        ...
        useFrame(({ clock }, delta) => {
            ...
            const potentialPositionXZ = new THREE.Vector2(potentialPosition.x, potentialPosition.z);

            for (const project of PROJECTS) {
                const projectPositionXZ = new THREE.Vector2(project.position.x, project.position.z);
                const distToIsland = potentialPositionXZ.distanceTo(projectPositionXZ);

                if (distToIsland < project.collisionRadius) {
                    const pushoutDirection = potentialPositionXZ.clone().sub(projectPositionXZ).normalize();
                    const correctedPositionXZ = projectPositionXZ.clone().add(pushoutDirection.multiplyScalar(project.collisionRadius));

                    physics.position.set(correctedPositionXZ.x, potentialPosition.y, correctedPositionXZ.y);
                    physics.velocity.multiplyScalar(0.5);

                    collisionDetected = true;
                    inSlowdownZone = false; // Override slowdown if hard collision occurs
                    break;
                }

                if (distToIsland < project.slowdownRadius) {
                    inSlowdownZone = true;
                }
            }

            if (inSlowdownZone) {
                physics.velocity.multiplyScalar(0.997);
            }
