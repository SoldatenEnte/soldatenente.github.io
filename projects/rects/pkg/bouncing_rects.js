/* @ts-self-types="./bouncing_rects.d.ts" */
 ...
 * @returns {WasmEngine}
 */
export function create_rects(count, mesh_id) {
    const ret = wasm.create_rects(count, mesh_id);
...
/**
 * The unit quad the instanced draw expands. Built here rather than reusing
 * `mesh_quad_2d` because the GPU-sim pipeline binds the 3D vertex layout:
 * 48-byte stride of position, normal, uv, colour.
 *
 * Wound counter-clockwise facing +Z, which is toward the camera — the pipeline
 * culls back faces.
 * @returns {object}
 */
export function mesh_unit_quad() {
    const ret = wasm.mesh_unit_quad();
    return ret;
}
...
async function __wbg_init(module_or_path, memory) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('bouncing_rects_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports(memory);
