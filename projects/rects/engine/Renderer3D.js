...
export class Renderer3D {
  constructor(renderer) {
    ...
    this.lastStats = {
      ...
      // Real GPU-side duration of the render pass, from timestamp queries.
      // Unlike gpuExecutionTimeMs (see the submit path) this is measured by the
      // GPU's own clock and is unaffected by vsync backpressure, so it stays
      // meaningful on a scene that is comfortably hitting its refresh rate --
      // which is exactly when the wall-clock figure stops telling you anything.
      // Stays 0 when the adapter has no `timestamp-query`.
      gpuRenderPassMs: 0,
      gpuComputePassMs: 0,
      gpuCullPassMs: 0,
    };
    // Timestamp-query plumbing; null when unsupported. Two query sets of two
    // timestamps (begin/end of the compute pass and of the render pass).
    this.timestampQuerySet = null;
    this.timestampResolveBuffer = null;
    // Readback is asynchronous and a mapped buffer cannot be written by the
    // GPU, so a single staging buffer would stall the pipeline every frame.
    // A small ring lets a frame's results land while later frames keep going.
  ...
  /// Turn on GPU timestamp instrumentation. Off by default: it adds a resolve
  /// and a buffer copy to every frame, which is not something a shipping scene
  /// should pay for, and it is a no-op on an adapter without the feature.
  ///
  /// Returns whether instrumentation is actually active afterwards, so a caller
  /// can tell "measured 0 ms" apart from "not measuring".
  enableGPUTimestamps(on = true) {
    ...
    if (!this.timestampQuerySet) {
      // 3 passes x (begin, end): sim compute, render, cull compute.
  ...
  /// Compact GPU-driven instances down to the ones intersecting the view
  /// frustum before drawing them, via a compute pass and an indirect draw.
  ///
  /// Off by default, and opt-in per shader rather than transparent, because it
  /// changes what `@builtin(instance_index)` means: after compaction an
  /// instance's index is its position in the visible list, not its identity in
  /// the simulation. A shader that derives anything stable-per-particle from
  /// `instance_index` -- a colour, a spawn position, a fixed rotation -- will
  /// see it change every time the camera moves, which looks like the whole
  /// scene reshuffling. Such shaders must read `@location(8) orig_index`
  /// instead, which carries the original index through compaction and is
  /// identity when culling is off, so one shader works either way.
  setFrustumCulling(on) {
    this.frustumCulling = !!on;
  }

  /// Six frustum planes in world space from a view-projection matrix
  /// (Gribb-Hartmann), normalised so plane-point distances come out in world
  /// units and can be compared against an instance's bounding radius.
  ///
  /// `m` is column-major as WGSL stores mat4x4, i.e. m[col * 4 + row], and the
  /// near plane is z > 0 rather than z > -w: WebGPU clip space is [0, w] in z,
  /// not OpenGL's [-w, w]. Using the GL form here silently culls everything in
  /// front of the camera.
  static frustumPlanes(m, out) {
    ...
    const planes = [
      [rw[0] + rx[0], rw[1] + rx[1], rw[2] + rx[2], rw[3] + rx[3]], // left
      [rw[0] - rx[0], rw[1] - rx[1], rw[2] - rx[2], rw[3] - rx[3]], // right
      [rw[0] + ry[0], rw[1] + ry[1], rw[2] + ry[2], rw[3] + ry[3]], // bottom
      [rw[0] - ry[0], rw[1] - ry[1], rw[2] - ry[2], rw[3] - ry[3]], // top
      [rz[0], rz[1], rz[2], rz[3]], // near
      [rw[0] - rz[0], rw[1] - rz[1], rw[2] - rz[2], rw[3] - rz[3]], // far
    ];
    for (let i = 0; i < 6; i++) {
      const p = planes[i];
  ...
  /// Compaction pass behind setFrustumCulling().
  ///
  /// Culling by a bounding *sphere* rather than the exact box is deliberate:
  /// the instance's rotation is a per-shader concern the engine does not know
  /// about, and a sphere is rotation-invariant, so this stays correct for any
  /// orientation a render shader chooses to apply. It is conservative at the
  /// frustum edge, which costs a few instances that would have been rejected by
  /// an exact test -- far cheaper than being wrong and popping geometry out at
  /// the screen border.
  static CullWGSL = `
    struct Particle {
    ...
    // Layout of a drawIndexedIndirect argument block. instance_count is the
    // compaction counter, which is why it has to be atomic.
    struct DrawArgs {
        index_count: u32,
    ...
    fn cs_cull(@builtin(global_invocation_id) id: vec3<u32>) {
        ...
        // Compaction is unordered: instances land in the visible list in
        // whatever order the atomic hands out slots, not in index order. That
        // is fine for an opaque depth-tested draw and is the reason the
        // original index has to travel alongside the particle.
  ...
  /// Allocate the culling resources for one sim. Lazy: a scene that never turns
  /// culling on never pays for the second particle buffer, which is the same
  /// size as the first and is the largest allocation here.
  initGPUSimCulling(state) {
    ...
    state.indirectBuffer = this.device.createBuffer({
      ...
      usage:
        ...
        // So the surviving instance count can be reported. The draw itself
        // never needs this -- the count stays on the GPU -- but a culling
        // feature you cannot observe is a culling feature you cannot trust.
        GPUBufferUsage.COPY_SRC,
    });
    // Small ring so reading the count never blocks the frame that produced it.
  ...
  initGPUSim(entId, maxInstances, shaderType) {
    ...
    // The identity permutation, bound at vertex slot 2 whenever culling is off
    // so that `orig_index` means the same thing in both modes and a shader
    // needs only one code path. Written once; it never changes.
    //
    // Uploaded in chunks for the same reason the particle seed above is: one
    // contiguous typed array over every instance is the allocation that fails
    // first as a scene scales up.
    ...
    const renderPipeline = this.device.createRenderPipeline({
      ...
      vertex: {
        ...
        buffers: [
          ...
          // Original simulation index, surviving compaction. A shader that does
          // not declare location 8 simply ignores this buffer -- an unconsumed
          // vertex attribute is valid -- so existing sim shaders are unaffected.
          {
            arrayStride: 4,
            stepMode: "instance",
  ...
  render(world, appDt) {
    ...
    if (gpuSims.length > 0) {
      ...
      // Culling reads what the simulation just wrote, so it has to be a
      // separate pass: ordering between dispatches inside one compute pass is
      // not a memory guarantee, and sharing a pass here reads last frame's
      // positions on some drivers and this frame's on others.
      if (this.frustumCulling) {
        ...
        for (let i = 0; i < gpuSims.length; i++) {
          ...
          for (let j = 0; j < view.len; j++) {
            ...
            // Reset the indirect block every frame; the compute pass counts up
            // from zero into instance_count. queue.writeBuffer is ordered
            // against the submit that follows it, so this lands first.
            ...
            // count is a u32 in the shader but this staging array is f32, so it
            // is written through a u32 view of the same bytes.
            new Uint32Array(this.cullParamsHost.buffer, 96, 1)[0] = n;
            // sqrt(3)/2: the bounding-sphere radius of a unit cube, which is
            // the largest mesh a sim instance is expected to be. Conservative
        ...
        // Recorded after the pass ends so the copy sees the final count.
        ...
        for (let j = 0; j < view.len; j++) {
          ...
          if (state && mesh) {
            ...
            if (mesh.indexBuffer) {
              pass.setIndexBuffer(mesh.indexBuffer, "uint32");
              if (culled) {
                // The instance count lives on the GPU -- the CPU never learns
                // it, which is the point: no readback, no stall, no frame of
                // latency.
    ...
    // Resolve + copy has to be recorded into this same encoder, before finish().
    let readSlot = null;
    if (this.timestampsEnabled) {
    ...
    // Whether a compute pass ran this frame at all -- queries 0/1 hold stale
    // values from an earlier frame otherwise, which would read as a phantom
    // compute cost on a scene that has no GPU sims.
    ...
    if (readSlot) {
      readSlot.buffer.mapAsync(GPUMapMode.READ).then(
        () => {
          ...
          readSlot.busy = false;
          // Nanoseconds on the GPU's own clock. A negative or absurd delta
          // means the timestamps straddled a clock reset -- drop it rather
          // than let one bad sample poison a rolling average.
          const span = (a, b) => {
