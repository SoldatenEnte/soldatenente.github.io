 ...
 * @returns {WasmEngine}
 */
export function create_field(side, mesh_id) {
    const ret = wasm.create_field(side, mesh_id);
 ...
 * @returns {string}
 */
export function schedule_json(engine) {
    let deferred1_0;
    let deferred1_1;
    try {
        _assertClass(engine, WasmEngine);
        const ret = wasm.schedule_json(engine.__wbg_ptr);
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
 ...
 */
export function set_parallel(engine, parallel) {
    _assertClass(engine, WasmEngine);
    wasm.set_parallel(engine.__wbg_ptr, parallel);
}
 ...
 */
export function trace_data(engine) {
    _assertClass(engine, WasmEngine);
    const ret = wasm.trace_data(engine.__wbg_ptr);
