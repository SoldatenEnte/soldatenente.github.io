/* @ts-self-types="./murmuration.d.ts" */
...
/**
 * CPU/ECS mode: `count` real entities, simulated by `sys_flow` every tick.
 * @param {number} count
 * @param {number} mesh_id
 * @param {number} cube_scale
 * @returns {WasmEngine}
 */
export function create_swarm_cpu(count, mesh_id, cube_scale) {
    const ret = wasm.create_swarm_cpu(count, mesh_id, cube_scale);
    return WasmEngine.__wrap(ret);
}

/**
 * GPU mode: one entity holding a `GPUDrivenSimulation` config. The cubes live
 * in a storage buffer and never touch the ECS. Same field, same look — the
 ...
 * @returns {WasmEngine}
 */
export function create_swarm_gpu(count, mesh_id, cube_scale) {
    const ret = wasm.create_swarm_gpu(count, mesh_id, cube_scale);
    return WasmEngine.__wrap(ret);
}
...
async function __wbg_init(module_or_path, memory) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('murmuration_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports(memory);
