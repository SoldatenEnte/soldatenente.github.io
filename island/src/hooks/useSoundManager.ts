...
class SoundManager {
    ...
    private sailingSound: Howl;
    private isInitialized: boolean = false;

    constructor() {
        this.music = new Howl({
            src: [asset('audio/calm-music.mp3')], // Use helper here
            loop: true,
            volume: 0.0,
        });

        this.ambientSound = new Howl({
            src: [asset('audio/ocean-ambient.mp3')], // Use helper here
            loop: true,
            volume: AMBIENT_BASE_VOLUME,
        });

        this.sailingSound = new Howl({
            src: [asset('audio/ship-sailing.mp3')], // Use helper here
            loop: true,
            volume: 0.0,
        });
    }
    ...
    private playAmbient() {
        ...
        if (!this.sailingSound.playing()) this.sailingSound.play();
    }
